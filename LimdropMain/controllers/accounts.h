...
class accounts:public drogon::HttpController<accounts>
{
  public:
    METHOD_LIST_BEGIN
		...
		METHOD_ADD(accounts::loginAccount, "/login?uname={1}&pass={2}&num={3}", Post);
    ...
    METHOD_LIST_END
		...
		void loginAccount(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback, std::string uname, std::string pass, int num);
