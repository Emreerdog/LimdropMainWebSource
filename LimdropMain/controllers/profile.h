...
class profile:public drogon::HttpController<profile>
{
  public:
    ...
    METHOD_ADD(profile::addAddress, "/addAddress?city={1}&address={2}&phoneNumber={3}&zipcode={4}", Post);
    ...
    void removeAddress(const HttpRequestPtr& req,std::function<void (const HttpResponsePtr &)> &&callback, std::string profileKey);
    void showAddress(const HttpRequestPtr& req,std::function<void (const HttpResponsePtr &)> &&callback, std::string profileKey);
