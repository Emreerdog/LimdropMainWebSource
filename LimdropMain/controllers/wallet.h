...
class wallet:public drogon::HttpController<wallet>
{
  public:
    ...
    METHOD_ADD(wallet::addcard,"/addcard?cardholderName={1}&cardNumber={1}&expireMonth={3}&expireYear={4}&cvc={5}", Post);//path is /wallet/{arg1}/{arg2}/list
    ...
    void addcard(const HttpRequestPtr& req,std::function<void (const HttpResponsePtr &)> &&callback, std::string holderName, std::string cardNumber, std::string expireMonth, std::string expireYear, std::string cvc);
};
